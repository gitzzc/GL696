...
static xComPortHandle xPort2 = NULL;
static xComPortHandle xPort3 = NULL;
...
void vAltStartComTestTasks( unsigned portBASE_TYPE uxPriority, eBaud ulBaudRate, unsigned portBASE_TYPE uxLED )
{
	/* Initialise the com port then spawn the Rx and Tx tasks. */
	uxBaseLED = uxLED;
	//xSerialPortInitMinimal( ulBaudRate, comBUFFER_LEN );
	xPort = xSerialPortInit( serCOM1, ulBaudRate, serNO_PARITY, serBITS_8, serSTOP_1, comBUFFER_LEN );
	xPort2 = xSerialPortInit( serCOM2, ulBaudRate, serNO_PARITY, serBITS_8, serSTOP_1, comBUFFER_LEN );
	xPort3 = xSerialPortInit( serCOM3, ulBaudRate, serNO_PARITY, serBITS_8, serSTOP_1, comBUFFER_LEN );
...
portTickType xTimeToWait;
	...
	for( ;; )
	{
		...
		for( cByteToSend = comFIRST_BYTE; cByteToSend <= comLAST_BYTE; cByteToSend++ )
		{
			...
			if( xSerialPutChar( xPort2, cByteToSend, comNO_BLOCK ) == pdPASS )
			{
				vParTestToggleLED( uxBaseLED + comTX_LED_OFFSET );
			}
			if( xSerialPutChar( xPort3, cByteToSend, comNO_BLOCK ) == pdPASS )
			{
				vParTestToggleLED( uxBaseLED + comTX_LED_OFFSET );
			}
		}
	...
	for( ;; )
	{
		...
		for( cExpectedByte = comFIRST_BYTE; cExpectedByte <= comLAST_BYTE; /*cExpectedByte++*/ )
		{
			...
			if( xSerialGetChar( xPort, &cByteRxed, comRX_BLOCK_TIME ) )
			{
				...
				//ADC_Get(ADC_Channel_8,buf,16);
				//xSerialPutChar( xPort, buf[0]>>8, comNO_BLOCK );
				//xSerialPutChar( xPort, buf[0]&0xff, comNO_BLOCK );
			}
		}
