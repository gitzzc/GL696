...
{
	xPort = xSerialPortInit( serCOM1, ser115200, serNO_PARITY, serBITS_8, serSTOP_1, 256 );
}
...
int get_frame(void)
{
	...
	if ( xSerialIsArrive(xPort) ){	       	//检测桢接收状徿	
		mb_rx_len = xSerialGet(xPort,mb_rx_buf,sizeof(mb_rx_buf),configTICK_RATE_HZ/100);
		if ( mb_rx_len < 3) 
			return 0;
...
void mb_send_frame(uint8_t *buf, uint8_t len)
{
	...
	crc16 = mb_CRC16(buf,len);
	buf[len++] = crc16 & 0xFF;
	buf[len++] = (crc16 >> 8) & 0xFF;
	vSerialPut( xPort, buf, len);
...
char preset_multiple_regs(void)
{
  ...
  for(reg_count=0;reg_count<write_len;){
		...
		switch( reg_addr ) {
			...
			case MB_DAC6:
			case MB_DAC7:
				PWM_DAC_Set(reg_addr - MB_DAC0, reg_value);
				break;
/*			case MB_FD110A:
