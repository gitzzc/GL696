...
#define partstFIRST_LED			GPIO_Pin_2
...
GPIO_InitTypeDef GPIO_InitStructure;
	...
	/* Configure PD.00 as output push-pull */
...
unsigned portSHORT usBit;
	...
	vTaskSuspendAll();
	{
		if( uxLED < partstMAX_OUTPUT_LED )
		{
			...
			{
				usOutputValue &= ~usBit;
				GPIO_WriteBit( GPIOB, usBit, Bit_RESET);
			}
			else
			{
				usOutputValue |= usBit;
				GPIO_WriteBit( GPIOB, usBit, Bit_SET);
			}
...
portBASE_TYPE xGetLEDState( unsigned portBASE_TYPE uxLED )
{
	if( uxLED < partstMAX_OUTPUT_LED )
	{
		return ( portBASE_TYPE ) (usOutputValue >> uxLED)&0x01;
	}
	else
	{
		return 0;
